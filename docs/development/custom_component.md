...
- `@component_action`: Validates lifecycle state before execution. **Must be annotated to return `Tuple[bool, str]`** (aliased as `ActionReturnType`) — the bool reports success, the string carries a result or an error message.
